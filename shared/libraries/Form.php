	...
	private function _linked_menu($field , $default)
	{
		...
		if ($default)
		{
			...
			foreach ($default as $v)
			{
				...
				foreach ($k as $key => & $kk)
				{
					if ($options[4] && $key === $options[2] - 1)
					{
						$kk = $ci->db->select('title')->where('id', $kk)->get($ci->db->dbprefix('u_m_'.strstr($options[0], '_class', true)))->row()->title;
					}
					else
					{
		...
		$html .= '<script language="javascript">$(".linked_menu_' . $options[0] . '").ld({ajaxOptions : {"url" : "' . 
			     backend_url('ld/json/' . $options[0] . '/' . $options[1] . '/' . $options[4]) . '"},style : {"width" : 120},field:{region_id:"classid",region_name:"' . 
