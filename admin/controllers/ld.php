...
class Ld extends CI_Controller
{
     ...
     * @return  void
     */
	public function json($model = '' , $name = '', $isrecord = 0)
	{
		if ( ! $model OR ! $name)
		{
			return;
		...
		//扩展：获取某分类下的记录
		if ($isrecord && $json_str == '[]') {
			$json = $this->db->select('id as classid, title')
							->where('classid', $parentid)
							->get($this->db->dbprefix('u_m_'.strstr($model, '_class', true)))
							->result_array();
